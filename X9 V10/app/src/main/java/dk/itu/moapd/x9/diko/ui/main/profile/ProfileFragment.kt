    ...
    */
    ...
    private fun setupUI() = with(binding) {
        textFieldUsername.editText?.doOnTextChanged { inputText, _, _, _ ->
            if (inputText != null && inputText.trim()!="Guest" && inputText.trim() != "")
            userViewModel.setUsername(inputText.toString())
        }

        buttonCheck.setOnClickListener {
            val userId = userRepository.currentUserId() ?: return@setOnClickListener
            if (userViewModel.username.value != "Guest" && userViewModel.username.value != "") {
            ...
            }else showSnackBar(getString(R.string.pick_different))
        }
    }
    ...
    private fun updateUserUI() {
        ...
        when (name) {
            ...
            "Guest" -> {
                ...
                binding.guestName.visibility = View.GONE
                binding.textGuests.visibility = View.GONE
                binding.textPutUsername.visibility = View.GONE
                binding.textReportStats.visibility = View.VISIBLE
            }
        }
    }
