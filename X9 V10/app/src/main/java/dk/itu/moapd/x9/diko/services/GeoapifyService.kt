...
object GeoapifyService {
    ...
    /**
     * The Geoapify API service provides Java code on the logic needed to connect to their API.
     * I used the help of AI to convert it into Kotlin and changed the structure to reflect my needs.
     */
    private val client = OkHttpClient()

    suspend fun autocomplete(query: String): List<Triple<String, Double, Double>> = withContext(Dispatchers.IO) {
        // Autocompletes a location address using the Geoapify API and gets equivalent geographical coordinates.
    ...
    suspend fun reverseGeocode(lat: Double, lon: Double): Triple<String, Double, Double>? = withContext(Dispatchers.IO) {
        // Reverse geocodes the given coordinates to a street address using the Geoapify API.
        val geoapifyApiKey = BuildConfig.GEOAPIFY_API_KEY
